            ...
            <div key={planId} className="">
              <Card
                className={`relative flex flex-col transition-all duration-300 ${
                  selectedPlan === planId ? "ring-gray-300 shadow-md" : "hover:shadow-sm"
                } ${planId === "pro_plan" ? "md:scale-102 z-10" : ""} ${planId === currentPlan ? "border-gray-300" : ""}`}
              >
                <GlowingEffect
                  spread={40}
                  glow={true}
                ...
                <CardHeader className="pb-4">
                  <CardTitle
                    className={`text-xl ${planId === "pro_plan" ? "coppergroup-gradient-text" : "text-gray-900"}`}
                  >
                    {plan.name}
                    {planId === "pro_plan" && <Star className="ml-1 h-4 w-4 text-yellow-400 inline-block" />}
                  </CardTitle>
                  <CardDescription>
                    {plan.price === 0 ? "Free" : `$${plan.price}/${plan.billing_cycle}`}
                  </CardDescription>
                </CardHeader>
                <CardContent>
