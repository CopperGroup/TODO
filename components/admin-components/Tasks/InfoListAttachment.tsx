...
import type { PopulatedTaskType, PopulatedTeamType, PopulatedColumnType, UserType } from "@/lib/types"
...
const InfoListAttachment: React.FC<InfoListAttachmentProps> = ({
  ...
  const handleSubtaskColumnChange = (subtaskId: string, columnId: string) => {
  ...
  const handleLinkedTaskAdd = (linkedTaskId: string) => {
    onUpdate({ ...task, linkedTasks: [...task.linkedTasks, linkedTaskId] })
  }

  const handleLinkedTaskRemove = (linkedTaskId: string) => {
    onUpdate({ ...task, linkedTasks: task.linkedTasks.filter((id) => id !== linkedTaskId) })
  }

  const handleLabelAdd = (newLabel: string) => {
    onUpdate({ ...task, labels: [...task.labels, newLabel] })
    setIsAddingLabel(false)
  }

  const handleLabelRemove = (labelToRemove: string) => {
    onUpdate({ ...task, labels: task.labels.filter((label) => label !== labelToRemove) })
  }

  const handleAssigneeAdd = (userId: string) => {
    const newAssignee = team.members.find((member) => member.user._id === userId)?.user
    if (newAssignee) {
      onUpdate({ ...task, assignedTo: [...task.assignedTo, newAssignee] })
    }
  }

  const handleAssigneeRemove = (userId: string) => {
    onUpdate({ ...task, assignedTo: task.assignedTo.filter((assignee) => assignee._id !== userId) })
  }
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {task.assignedTo.map((assignee: UserType) => (
              <Badge
                key={assignee._id}
                variant="secondary"
        ...
        <div className="space-y-3">
          ...
          <Select value={task.column as string} onValueChange={(value) => onUpdate({ ...task, column: value })}>
        ...
        <AddSubtask
          ...
          parentTaskColumnId={task.column as string}
          boardId={boardId}
        />
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {task.linkedTasks.map((linkedTaskId) => {
              const linkedTask = allTasks.find((t) => t._id === linkedTaskId)
              return linkedTask ? (
                <LinkedTaskItem key={linkedTaskId} task={linkedTask} handleUnlinkTask={handleLinkedTaskRemove} />
              ) : null
            })}
          </div>
          <LinkedTaskSelectionCombobox
            ...
            linkedTaskIds={task.linkedTasks}
