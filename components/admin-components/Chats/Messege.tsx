...
export default function MessegeItem({
  ...
  return (
    <Messege key={messege._id} className={`flex ${isOwnMessege ? "justify-end" : "justify-start"} mb-6 w-full`}>
      <div className={`flex ${isOwnMessege ? "flex-row-reverse" : "flex-row"} items-start gap-3 max-w-[80%]`}>
        ...
        <div className={`flex flex-col ${isOwnMessege ? "items-end" : "items-start"} max-w-full`}>
          ...
          >
            {messegeType === "Default" ? (
              <p className="whitespace-pre-wrap break-words">{messege.content}</p>
            ) : (
              <div
