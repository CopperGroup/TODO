  ...
  return (
    <PopoverTrigger asChild>
      <Button
        ...
        aria-expanded={open}
        className={cn(
          "w-full justify-between bg-neutral-700 border-neutral-600 font-normal text-muted-foreground hover:bg-neutral-600 hover:text-white",
          className
        )}
      >
...
export function TimeSelectContent({ className }: TimeSelectContentProps) {
  ...
  return (
    <PopoverContent className={cn("w-[180px] p-0 bg-neutral-800 border-neutral-700", className)}>
          ...
          <div className="px-1">
            {hours.map((hour) => (
              <button
                key={hour}
                className={cn(
                  "w-full px-2 py-1 text-sm text-left hover:bg-neutral-600 transition-colors rounded-sm text-zinc-200",
                  selectedHour === hour && "coppergroup-gradient text-white font-medium"
                )}
          ...
          <div className="px-1">
            {minutes.map((minute) => (
              <button
                ...
                className={cn(
                  "w-full px-2 py-1 text-sm text-left hover:bg-neutral-600 transition-colors rounded-sm text-zinc-200",
                  selectedMinute === minute && "coppergroup-gradient text-white font-medium"
                )}
